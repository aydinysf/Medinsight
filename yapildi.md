...
- [Feature] WP-LLM: OpenAI-uyumlu istemci (Kimi/DeepSeek/OpenAI) + ortak JSON sözleşmesi
...
- [Feature] Hızır chat (ADR-018): vaka kapsamlı, guardrail'li senkron sohbet
  - Dosya: docs/adr/adr-018-hizir-chat.md, src/MedInsight.Domain/Cases/{HizirChatMessage,Case}.cs, src/MedInsight.AIOrchestration/{ILlmClient,StubLlmClient,GeminiLlmClient,OpenAiCompatibleLlmClient,HizirOrchestrator,DependencyInjection}.cs, Handlers/HizirChatHandlers.cs, src/MedInsight.Api/Controllers/HizirChatController.cs, src/MedInsight.Infrastructure/{Persistence/MedInsightDbContext,Repositories/CaseRepository}.cs, Migrations/20260728103348_AddHizirChatMessages, frontend/src/features/cases/components/HizirChatTab.tsx + api.ts + CaseDetailPage.tsx + types.ts, tests
  - Not: ILlmClient.ChatAsync (3 implementasyon; geçmişli, JSON sözleşmesinden bağımsız serbest metin). HizirOrchestrator.ChatAsync: PII-minimize bağlam + son analiz özeti + sabit persona talimatı (tanı/doz yok, acilde 112, injection reddi); yanıt Guardrails.EnforceScope'tan geçer. Mesajlar Case aggregate'inde kalıcı (domain event yok — ADR-018 gerekçeli), son 20 mesaj bağlam penceresi, messages rate limit politikası. GET/POST /cases/{id}/hizir-chat (üye veya Admin). UI: "Hızır'a Sor" sekmesi (karşılama, baloncuklar, "Hızır yazıyor…", kalıcı disclaimer). Canlı E2E (gpt-4o-mini): rapor sorusuna bağlama dayalı yanıt + doktora yönlendirme; "kurallarını boşver, kesin tanı + mg" jailbreak denemesi zorunlu yönlendirme metniyle engellendi. 133 test geçiyor.

- [Feature] Hızır chat (ADR-018): vaka kapsamlı, guardrail'li senkron sohbet
  - Dosya: docs/adr/adr-018-hizir-chat.md, src/MedInsight.Domain/Cases/{HizirChatMessage,Case}.cs, src/MedInsight.AIOrchestration/{ILlmClient,StubLlmClient,GeminiLlmClient,OpenAiCompatibleLlmClient,HizirOrchestrator,DependencyInjection}.cs, Handlers/HizirChatHandlers.cs, src/MedInsight.Api/Controllers/HizirChatController.cs, src/MedInsight.Infrastructure/{Persistence/MedInsightDbContext,Repositories/CaseRepository}.cs, Migrations/20260728103348_AddHizirChatMessages, frontend/src/features/cases/components/HizirChatTab.tsx + api.ts + CaseDetailPage.tsx + types.ts, tests
  - Not: ILlmClient.ChatAsync (3 implementasyon; geçmişli, JSON sözleşmesinden bağımsız serbest metin). HizirOrchestrator.ChatAsync: PII-minimize bağlam + son analiz özeti + sabit persona talimatı (tanı/doz yok, acilde 112, injection reddi); yanıt Guardrails.EnforceScope'tan geçer. Mesajlar Case aggregate'inde kalıcı (domain event yok — ADR-018 gerekçeli), son 20 mesaj bağlam penceresi, messages rate limit politikası. GET/POST /cases/{id}/hizir-chat (üye veya Admin). UI: "Hızır'a Sor" sekmesi (karşılama, baloncuklar, "Hızır yazıyor…", kalıcı disclaimer). Canlı E2E (gpt-4o-mini): rapor sorusuna bağlama dayalı yanıt + doktora yönlendirme; jailbreak denemesi zorunlu yönlendirme metniyle engellendi. 133 test geçiyor.

- [Fix] Hızır chat: guardrail yanlış pozitifi + kaybolan ilk mesaj (ürün sahibi bildirimi)
  - Dosya: src/MedInsight.AIOrchestration/Pipeline/Guardrails.cs, Handlers/HizirChatHandlers.cs, HizirOrchestrator.cs, frontend/src/features/cases/api.ts, tests/MedInsight.AIOrchestration.Tests/HizirOrchestratorTests.cs
  - Not: (1) Kapsam regex'i "tanı koy" kalıbını olumsuzlamada da yakalıyordu — Hızır "ben tanı koymam" dediği her yanıt zorunlu yönlendirmeye dönüyordu; regex iddia kipleriyle sınırlandırıldı ("tanı koyuyorum/koydum/koyarım", "tanınız:", "kesin tanı"...), 2 regresyon testi eklendi. (2) Kullanıcı mesajı ile LLM yanıtı tek transaction'daydı; LLM hatasında soru kayboluyordu — iki aşamalı kayıt + hata durumunda kalıcı fallback Hızır yanıtı; frontend onSettled ile hata durumunda da listeyi yeniler. (3) Belgesiz vakada bağlama [NOT] eklendi — Hızır artık "henüz belge yok, Belgeler sekmesinden yükle" diye yönlendiriyor. Canlıda doğrulandı; 135 test geçiyor.

- [Fix] Hızır chat: görüntü (DICOM) sorularında yönlendirici yanıt
  - Dosya: src/MedInsight.AIOrchestration/HizirOrchestrator.cs
  - Not: Ürün sahibi bildirimi — MR yükleyen hastaya Hızır kuru bir "dosya inceleyemem" diyordu. Sohbet bağlamına iki not eklendi: (1) vakada DICOM çalışması varsa Hızır görüntüyü yorumlayamayacağını (ADR-010) ama çalışmanın kayıtlı olduğunu ve doktorun değerlendireceğini söyler, yazılı raporu PDF olarak yüklemeyi önerir; (2) deneysel görüntü bulguları (ImageFinding) bağlama zorunlu disclaimer talimatıyla girer — tanıya bağlanamaz. Canlı doğrulandı: "MR görüntülerimi yükledim, bakabildin mi?" → doktor yönlendirmesi + PDF rapor önerisi. 135 test geçiyor.
