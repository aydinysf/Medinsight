...
public class RegisterPatientHandlerTests
{
    ...
    private RegisterPatientHandler Handler => new(_users, _patients);

    [Fact]
    public async Task Kullanici_ve_hasta_birlikte_olusur()
    {
        var dto = await Handler.HandleAsync(new RegisterPatient("Test Hasta", "Test.Hasta@Example.com", new DateOnly(1980, 4, 12), Sex.Female));
    ...
    {
        var dto = await Handler.HandleAsync(new RegisterPatient("Test Hasta", "Test.Hasta@Example.com", null, null));
    ...
    {
        await Handler.HandleAsync(new RegisterPatient("Birinci", "ayni@example.com", null, null));

        await Assert.ThrowsAsync<DomainException>(() =>
            Handler.HandleAsync(new RegisterPatient("İkinci", "AYNI@example.com", null, null)));
    ...
    {
        var dto = await Handler.HandleAsync(new RegisterPatient("Test Hasta", "x@example.com", null, null));
